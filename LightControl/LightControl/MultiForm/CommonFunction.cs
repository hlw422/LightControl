...
0xFF00	16进制数字	ASC码	ASC码	ASC码	根据具体协议	帧内容的按字节和校验
           */
            ...
            if (command_code == "02" || command_code == "2")//2017.10.31 命令码为2，则为定时上传内容
            {
                //开关状态
            ...
            else if(SendConvertString(receiveData[29].ToString(), 10, 16).PadLeft(2, '0')=="83")//开灯应答
            {
                DataRow dr = dtClientInfo.Select("F_IMEI='" + IMEI + "'")[0];
                dr["F_IP"] = IP;
                dr["F_IMEI"] = IMEI;
            ...
            else if(SendConvertString(receiveData[29].ToString(), 10, 16).PadLeft(2, '0') == "84")//关灯应答
            {
                DataRow dr = dtClientInfo.Select("F_IMEI='" + IMEI + "'")[0];
                dr["F_IP"] = IP;
                dr["F_IMEI"] = IMEI;
